...
object CSVProcessingParallelStreams extends IOApp.Simple {
  ...
  def readLegoSetStreamsParallely(fileName: String, p: LegoSet => Boolean, limit: Int): IO[List[LegoSet]] = {
    Files[IO]
      ...
      .through(text.lines)
      .parEvalMapUnbounded(s => IO(parseLegoSet(s))) // to make the line processing parallel
      .unNone
      .filter(p)
      .take(limit)
      .compile
  ...
  override def run: IO[Unit] = {
    ...
    readLegoSetStreamsParallely(fileName, _.year > 1970, 10)
      .flatMap(IO.println)
  }
